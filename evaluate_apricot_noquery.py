...
from apricot_noquery_core.utils_apricot_noquery import placement_to_scene
...
flags.DEFINE_string(
    "responses", None, "Path to folder containing responses."
)
...
def main(argv):
    ...
    result_folder_parent = Path(f"results/apricot_noquery-{model_tag}")
    if result_folder_parent.exists():
        print(f"WARNING: folder {result_folder_parent} already exists. Creating a new results folder.")
        result_folder_parent = Path(f"results/apricot_noquery-{FLAGS.model_tag}-{date_tag}")
    ...
    for fkey, fold_df_dict in fold_dict.items():
        ...
        for scene_id, scene_dict in test_batches.items():
            ...
            placements = response_dict["placements"]
            partial_scene = scene_dict["partial"]
            goal_scene = scene_dict["goal"]

            predicted_scene = placement_to_scene(partial_scene, placements)
